...
struct yield_expr: yield_expr_base<Sig>
{
    ...
    template<typename E>
    explicit yield_expr(E&& e_):
        e(std::forward<E>(e_))
    {}

    ret_type expr(call_params_type_t<Sig>& t) const override
    {
        return e(t);
    }
...
public:
    template<typename U>
    explicit co_return_handler_t(U&& u, std::shared_ptr<yield_expr_list<Sig>> yields_):
    ...
    return_of_t<Sig> call(trace_agent& /*agent*/, call_params_type_t<Sig>& params) override
    {
        ...
        if constexpr (requires {
                          std::declval<promise_type&>().yield_value(std::declval<value_type>());
                      })
        {
            for (auto& e: *yields)
            {
...
struct co_throw_handler_t
{
    ...
    explicit co_throw_handler_t(H&& h_):
        h(std::move(h_))
    {}
...
struct handle_co_yield
{
    ...
    static call_modifier<Matcher, modifier_tag, Parent>
        action(call_modifier<Matcher, modifier_tag, Parent>&& m, E&& e)
    {
        ...
        constexpr bool is_matching_type = std::invoke(
            []
            {
                if constexpr (is_coroutine && !is_void)
                {
                    ...
                    return requires(promise p, ret r)
                    {
                        p.yield_value(r);
                    };
                }
        ...
        static_assert(is_coroutine, "CO_YIELD when return type is not a coroutine");
...
struct handle_co_return
{
        ...
        action(call_modifier<Matcher, modifier_tag, Parent>&& m, H&& h)
    {
        ...
        static_assert(has_return || !has_co_return, "Multiple CO_RETURN does not make sense");
        ...
        constexpr bool valid = !has_return && !has_co_return && is_coroutine && is_matching_type &&
...
struct handle_co_throw
{
        ...
        action(call_modifier<Matcher, modifier_tag, Parent>&& m, H&& h)
    {
        ...
        using sigret = return_of_t<signature>;
        constexpr bool is_coroutine = trompeloeil::is_coroutine<sigret>::value;
        static_assert(is_coroutine, "Do not use CO_THROW from a normal function, use THROW");
        static_assert(!is_coroutine || !Parent::throws, "Multiple CO_THROW does not make sense");
        constexpr bool has_return = !std::is_same<co_return_type, void>::value;
        static_assert(!is_coroutine || !has_return, "CO_THROW and CO_RETURN does not make sense");
        ...
        if constexpr (valid)
        {
            ...
            m.matcher->return_handler_obj.reset(
                new ret_handler(std::move(handler), m.matcher->yield_expressions));
        }
        return {std::move(m).matcher};
    }
};
