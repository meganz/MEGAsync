...
struct regex_check
{
    ...
    public:
        ...
        string_helper(const S& s) noexcept:
            begin_(s.data()),
            end_(begin_ + s.length())
        {}

        constexpr string_helper(char const* s) noexcept:
            begin_(s),
    ...
    regex_check(std::regex&& re_, std::regex_constants::match_flag_type match_type_):
        re(std::move(re_)),
        match_type(match_type_)
    {}

    template<typename T>
    bool operator()(string_helper str, T const&) const
    {
...
{
    template<typename T>
    void operator()(std::ostream& os, T const& str) const
    {
...
template<typename Kind = wildcard,
         typename R =
             make_matcher_return<Kind, lambdas::regex_check, lambdas::regex_printer, std::string&&>>
auto re(std::string s,
...
{
    return make_matcher<Kind>(lambdas::regex_check(std::regex(s, opt), match_type),
...
template<typename Kind = wildcard,
         typename R =
             make_matcher_return<Kind, lambdas::regex_check, lambdas::regex_printer, std::string&&>>
auto re(std::string s, std::regex_constants::match_flag_type match_type)
    TROMPELOEIL_TRAILING_RETURN_TYPE(R)
{
