...
public:
    template<typename... U, typename = detail::enable_if_t<std::is_constructible<T, U...>::value>>
    deathwatched(U&&... u) noexcept(noexcept(T(std::declval<U>()...))):
        T(std::forward<U>(u)...)
    {}
    ...
    trompeloeil::lifetime_monitor*&
        trompeloeil_expect_death(trompeloeil::lifetime_monitor* monitor) const noexcept
    {
        auto lock = get_lock();
...
{
    template<typename T>
    lifetime_monitor(::trompeloeil::deathwatched<T> const& obj,
    ...
    template<typename... T>
    void set_sequence(T&&... t)
    {
        using handler = sequence_handler<sizeof...(T)>;
        auto seq =
            detail::make_unique<handler>(*sequences, invocation_name, loc, std::forward<T>(t)...);
...
deathwatched<T>::~deathwatched()
{
    ...
    std::ostringstream os;
    os << "Unexpected destruction of " << TROMPELOEIL_TYPE_ID_NAME(T) << "@" << this << '\n';
...
{
    explicit lifetime_monitor_modifier(std::unique_ptr<lifetime_monitor>&& p) noexcept:
        unique_ptr(std::move(p))
    {}

    template<typename... T, bool b = sequence_set>
    auto in_sequence(T&&... t) -> lifetime_monitor_modifier<true>
    {
        static_assert(!b,
...
{
    template<bool b>
    std::unique_ptr<trompeloeil::lifetime_monitor> operator+(lifetime_monitor_modifier<b>&& m) const
    {
        return std::move(m);
    }
};
...
#define TROMPELOEIL_REQUIRE_DESTRUCTION(obj) TROMPELOEIL_REQUIRE_DESTRUCTION_(obj, #obj)

#define TROMPELOEIL_REQUIRE_DESTRUCTION_(obj, obj_s) \
    std::unique_ptr<trompeloeil::expectation> TROMPELOEIL_CONCAT(trompeloeil_death_monitor_, \
                                                                 __LINE__) = \
...
#define TROMPELOEIL_NAMED_REQUIRE_DESTRUCTION_(prefix, obj, obj_s) \
    trompeloeil::lifetime_monitor_releaser{} + trompeloeil::lifetime_monitor_modifier<false> \
    { \
        ::trompeloeil::detail::make_unique<trompeloeil::lifetime_monitor>( \
            ...
            ::trompeloeil::location{__FILE__, static_cast<unsigned long>(__LINE__)}) \
    }
