...
QtObject {
    ...
    readonly property string only: qsTr("only")
    readonly property string pricePerMonthAfterTax: qsTr("%1 after tax")

    function discountLabel(n) {
        return qsTr("%1% off for %n month", "", n)
    }}
