    ...
    mGlobalError(BackupErrorCode::NONE)
{
    ...
    connect(&BackupsController::instance(),
            ...
            this,
            &BackupsModel::onBackupsCreationFinished);
    connect(&BackupsController::instance(),
            &BackupsController::backupFinished,
            this,
            &BackupsModel::onBackupFinished);
...
{
    if (!fromModel && mCheckAllState == Qt::CheckState::Unchecked &&
...
QString BackupsModel::getSdkErrorString() const
{
    ...
    auto itFound =
        std::find_if(mBackupFolderList.cbegin(),
                     mBackupFolderList.cend(),
...
void BackupsModel::insert(const QString& folder)
{
    ...
    BackupFolder* data =
        new BackupFolder(inputPath,
                         BackupsController::instance().getSyncNameFromPath(inputPath),
                         true,
                         this);
...
void BackupsModel::populateDefaultDirectoryList()
{
    ...
    for (auto type: qAsConst(defaultPaths))
    {
        ...
        {
            BackupFolder* folder =
                new BackupFolder(path,
                                 BackupsController::instance().getSyncNameFromPath(path),
                                 false,
                                 this);
...
void BackupsModel::updateSelectedAndTotalSize()
{
    ...
    for (auto backupFolderIt = mBackupFolderList.cbegin();
         backupFolderIt != mBackupFolderList.cend();
         ++backupFolderIt)
    {
...
{
    auto foundFolderIt = std::find_if(mBackupFolderList.cbegin(),
                                      mBackupFolderList.cend(),
                                      [&inputPath](const BackupFolder* const folder)
                                      {
    ...
    if (foundFolderIt != mBackupFolderList.cend())
    {
        ...
        {
            setData(
                index(static_cast<int>(std::distance(mBackupFolderList.cbegin(), foundFolderIt)),
                      0),
                QVariant(true),
                SELECTED_ROLE);
        }
...
bool BackupsModel::folderContainsOther(const QString& folder, const QString& other) const
{
    if (folder == other)
    {
        return true;
...
bool BackupsModel::isRelatedFolder(const QString& folder, const QString& existingPath) const
{
...
void BackupsModel::reviewConflicts()
{
    ...
    mConflictsSize =
        duplicatedCount + mRemoteCount + syncConflictCount + pathRelationCount + unavailableCount;
...
bool BackupsModel::existOtherRelatedFolder(const int currentRow)
{
    ...
    {
        if ((found = mBackupFolderList[conflictRow]->mSelected && conflictRow != currentRow &&
                     isRelatedFolder(folder, mBackupFolderList[conflictRow]->getFolder())))
        {
        ...
        if (mBackupFolderList[row]->mSelected && !mBackupFolderList[row]->mDone)
        {
            ...
            if (mBackupFolderList[row]->mError == BackupErrorCode::NONE &&
                ...
                BackupsController::instance().isLocalFolderSyncable(
                    mBackupFolderList[row]->getFolder(),
                    mega::MegaSync::TYPE_BACKUP,
                    message) != SyncController::CAN_SYNC)
            {
                QDir dir(mBackupFolderList[row]->getFolder());
                mBackupFolderList[row]->mError = dir.exists() ? BackupErrorCode::SYNC_CONFLICT :
                                                                BackupErrorCode::UNAVAILABLE_DIR;
            }
            else
            {
...
int BackupsModel::rename(const QString& folder, const QString& name)
{
    ...
    if (!hasError)
    {
        ...
        {
            hasError = (i != row && mBackupFolderList[row]->mSelected &&
                        name == mBackupFolderList[i]->mName);
            if (hasError)
            {
...
void BackupsModel::remove(const QString& folder)
{
    ...
    while (!found && item != mBackupFolderList.end())
    {
        ...
        {
            name = (*item)->mName;
            // QList::size is an int, so it is safe to cast
            // iterator_traits<_InputIter>::difference_type to int
...
void BackupsModel::change(const QString& oldFolder, const QString& newFolder)
{
    ...
    while (!found && item != mBackupFolderList.end())
    {
        if ((*item)->mSelected && (found = ((*item)->getFolder() == oldFolder)))
        {
            ...
            setData(index(getRow(oldFolder), 0),
                    QVariant(BackupsController::instance().getSyncNameFromPath(newFolder)),
                    NAME_ROLE);
    ...
    while (item != mBackupFolderList.end())
    {
        if ((*item)->mSelected)
        {
            ...
            else
            {
                if (resetErrors)
                {
                    setData(index(getRow((*item)->getFolder()), 0),
                            QVariant(BackupErrorCode::NONE),
                            ERROR_ROLE);
                }
                item++;
            }
    ...
    mSelectedFilterEnabled(false)
{
    ...
    connect(backupsModel(),
            &BackupsModel::backupsCreationFinished,
            this,
            &BackupsProxyModel::backupsCreationFinished);
}
