...
AddSyncManager::AddSyncManager()
{

}

void AddSyncManager::addSync(MegaHandle handle, bool disableUi)
{
    ...
    auto addSyncLambda = [overQuotaDialog, handle, disableUi, this]()
    {
        if(!overQuotaDialog || overQuotaDialog->result() == QDialog::Rejected)
        {
            ...
            DialogOpener::showDialog(mAddSyncDialog, this, &AddSyncManager::onAddSyncDialogFinished);
        }
    };
...
void AddSyncManager::onAddSyncDialogFinished(QPointer<BindFolderDialog> dialog)
{
    ...
    syncController->addSync(localFolderPath, handle, syncName, MegaSync::TYPE_TWOWAY);
}
