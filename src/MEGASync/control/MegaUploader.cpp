...
void MegaUploader::upload(QString path, const QString& nodeName, std::shared_ptr<MegaNode> parent, unsigned long long appDataID, const std::shared_ptr<TransferBatch>& transferBatch)
{
    ...
    string localPath = megaApi->getLocalPath(parent.get());
#ifdef WIN32
    ...
    if (localPath.size() && currentPath != destPath && megaApi->isSyncable(destPath.toUtf8().constData(), info.size()))
    {
        if (!destPath.startsWith(QFileInfo(currentPath).canonicalFilePath()))//to avoid recurses //note: destPath should have been cannonicalized already
        {
            ...
            QObject* detectFinish = new QObject();
            connect(detectFinish, &QObject::destroyed, this, [this, info]()
            {
                emit uploadRecursivelyIntoASyncedLocationFinished(info.absoluteFilePath());
            });

            std::function<void(QFileInfo, QString, std::shared_ptr<MegaNode>, unsigned long long)> uploadRecursivelyLambda
                    = std::bind(&MegaUploader::uploadRecursivelyIntoASyncedLocation, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);

            std::function<void()> startUploadRecursively = [=]()
            {
                uploadRecursivelyLambda(info, destPath, parent, appDataID);
                detectFinish->deleteLater();
            };

            mThreadPool->push(startUploadRecursively);
...
void MegaUploader::uploadRecursivelyIntoASyncedLocation(QFileInfo srcFileInfo, QString destPath, std::shared_ptr<MegaNode> parent, unsigned long long appDataID)
{
    ...
    if (!megaApi->isSyncable(destPath.toUtf8().constData(), srcFileInfo.size())) //if not syncable, do not copy locally, but simply upload: we don't want to create it localle nor override current files/folders
    {
        ...
        startUpload(srcPath, QString(), appDataID, parent.get(), nullptr);
        return;
    }
    ...
    else if (srcFileInfo.isDir()) // for folders we need to recurse
    {
        ...
        std::shared_ptr<MegaNode> newParent(megaApi->getNodeByPath(srcFileInfo.fileName().toUtf8().constData(), parent.get()));
        if (!newParent || !newParent->isFolder()) //for files it will leave the file and create a folder, same as regular upload
        {
            ...
            megaApi->createFolder(srcFileInfo.fileName().toUtf8().constData(), parent.get(), srl.get());
            srl->wait();
