...
long long Preferences::lastPublicHandleTimestamp()
{
    mutex.lock();
    assert(logged());
    long long value = getValue<long long>(lastPublicHandleTimestampKey, 0);
    mutex.unlock();
    return value;
}
...
MegaHandle Preferences::lastPublicHandle()
{
    mutex.lock();
    assert(logged());
    MegaHandle value = getValue<MegaHandle>(lastPublicHandleKey, (unsigned long long) mega::INVALID_HANDLE);
    mutex.unlock();
    return value;
}
