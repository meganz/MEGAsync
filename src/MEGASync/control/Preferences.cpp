...
void Preferences::getLastHandleInfo(MegaHandle &lastHandle, int &type, long long &timestamp)
{
    mutex.lock();
    assert(logged());
    timestamp = getValue(lastPublicHandleTimestampKey, 0);
    lastHandle = getValue(lastPublicHandleKey, (unsigned long long) mega::INVALID_HANDLE);
    type = getValue(lastPublicHandleTypeKey, (int)MegaApi::AFFILIATE_TYPE_INVALID);
    mutex.unlock();
}
